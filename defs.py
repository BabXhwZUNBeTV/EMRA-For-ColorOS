...
def extract_img():
    ...
    subprocess.run(["./payload-dumper-go", "-c", "8", "-o", "./", "-p",
                   "my_product,my_stock,my_bigball,my_heytap,system_ext", "payload.bin"])
...
def extract_files():
    try:
        ...
        output = subprocess.check_output(
            ["file", "my_product.img"]).decode("utf-8")
        ...
        if "EROFS filesystem" in output:
            ...
            subprocess.run(
                ["./extract.erofs", "-i", "my_product.img", "-x", "-T16"])
            subprocess.run(
                ["./extract.erofs", "-i", "my_stock.img", "-x", "-T16"])
            subprocess.run(
                ["./extract.erofs", "-i", "my_bigball.img", "-x", "-T16"])
            subprocess.run(
                ["./extract.erofs", "-i", "my_heytap.img", "-x", "-T16"])
            subprocess.run(
...
def delete_files_and_folders():
    """删除指定的文件和文件夹"""
    files_to_delete = ["payload.bin", "my_product.img", "my_stock.img",
                       "my_bigball.img", "my_heytap.img", "system_ext.img", "app_code_name.json"]
    folders_to_delete = ["output_apk", "update_apk", "update_name_apk", "config",
                         "my_heytap", "my_product", "my_stock", "system_ext", "my_bigball"]
